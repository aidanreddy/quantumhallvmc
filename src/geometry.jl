...
function crosszhat(V::Vector)::Vector{}
    return [V[2], -V[1]]
end
...
function geometry_setup(A1,A2,N1,N2,rl_cut,B_field,fermi_surface)
    ...
    G1=2pi*crosszhat(L2)/area
    G2=-2pi*crosszhat(L1)/area
    g1=2pi*crosszhat(A2)/(area/(N1*N2))
    g2=-2pi*crosszhat(A1)/(area/(N1*N2))
    mesh=reshape([i*G1.+j*G2 for i in 0:(N1-1), j in 0:(N2-1)], N1*N2)
    if B_field==false
        if fermi_surface==true
            mesh_raw=reshape([i*G1.+j*G2 for i in -N1:N1, j in -N2:N2], (2*N1+1)*(2*N2+1))
            mesh=mesh_raw[sortperm(norm.(mesh_raw))[1:N1*N2]]
        elseif fermi_surface==false
            mesh_raw=copy(mesh)
