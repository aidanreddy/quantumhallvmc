function sample(
    ...
    #### setup
    #for shifting coordinates to first supercell
    ...
    R_walk[1] = R
    Tl_walk = [0.0 + 0im for _ in 1:(walk_length)]
    ...
    deleteat!(g_vals, findall(x->x == [0.0, 0.0], g_vals))
    ...
    for i in axes(r_diffs, 1)
        ...
        gradx_u_pair_row, grady_u_pair_row = calc_dxdy_u_row(
            i, r_diffs[i, :], dx_u_func, dy_u_func
        )
        gradx_u_pair_mat[i, :] = gradx_u_pair_row
        grady_u_pair_mat[i, :] = grady_u_pair_row
        laplacian_u_pair_mat[i, :] = calc_laplacian_u_row(
            i, r_diffs[i, :], laplacian_u_func
        )
    end
    ...
    for I in 1:walk_length
        ...
        shift = random_shift
        ri_new = send_to_first_supercell(
            R[i, :] + shift, supercell_to_cart, cart_to_supercell
        ) #shift
        ...
        @assert !isnan(J_ratio)
        acceptance_ratio = abs(slater_ratio * J_ratio)^2
        accept = (rand() < acceptance_ratio)
        if accept
            r_diffs = copy(r_diffs_new)
            #update slater stuff
            D_inv = update_D_inv(i, delta_D_new_coli, D_inv) #Sherman-Morrison
            D[:, i] = D_new_coli
            O[:, :, i] = o_new_coli
            O_padded[:, :, i] = O_padded_new_coli
            pix_O_new_coli, piy_O_new_coli, pi_square_O_new_coli = calc_O_kin_col(
                o_new_coli, O_padded_new_coli, orbital_inputs
            )
            ...
            gradx_u_pair_row, grady_u_pair_row = calc_dxdy_u_row(
                i, r_diffs_new[i, :], dx_u_func, dy_u_func
            )
            gradx_u_pair_mat[i, :] = gradx_u_pair_row
            grady_u_pair_mat[i, :] = grady_u_pair_row
            gradx_u_pair_mat[:, i] = -gradx_u_pair_row
            grady_u_pair_mat[:, i] = -grady_u_pair_row
            laplacian_u_pair_row = calc_laplacian_u_row(
                i, r_diffs_new[i, :], laplacian_u_func
            ) #update both rows and columns of U_total matrices!!!!
            laplacian_u_pair_mat[i, :] = laplacian_u_pair_row
            laplacian_u_pair_mat[:, i] = laplacian_u_pair_row
            R = copy(R_new)
            if comp_el
                delta_v = calc_Coulomb_energy_change(
                    i, R_walk[I][i, :], R, η, area, v_long_g, l_vals, g_vals
                )
                T = calc_kinetic_energy(
                    pix_D,
                    piy_D,
...
function sample_opt(
    ...
    acceptance_rate = 0.0
    Δ = zeros(ComplexF64, NP) #logarithmic derivative of Ψ with respect to variational parameters
    Δ_conj_Δ = zeros(ComplexF64, (NP, NP))
    Δ_conj_el = zeros(ComplexF64, NP)
    ...
    for I in 1:(walk_length + burn)
        ...
        gradx_u_pair_row_new_i, grady_u_pair_row_new_i = calc_dxdy_u_row(
            i, r_diffs_new[i, :], dx_u_func, dy_u_func
        )
        accept = (rand() < acceptance_ratio)
        ...
        if accept
            r_diffs = copy(r_diffs_new)
            #update slater stuff
            D_inv_new = update_D_inv(i, delta_D_new_coli, D_inv) #Sherman-Morrison
        ...
        if I > burn
            ...
            if (I - burn) == 1
                if opt_J
                    for l in 2:size(R)[1], j in 1:(l - 1)
                        delta[1:NJ] += dparams_u_func(R[l, :] - R[j, :])
                    end
                end
                if opt_D
                ...
                if accept #proceed if particle was moved
                    if opt_J
                        for j in 1:ne
                            if j != i
                                delta[1:NJ] += dparams_u_func(R[i, :] - R[j, :]) #no factor of 1/2 is necessary here 
                                delta[1:NJ] -= dparams_u_func(r_prev[i, :] - r_prev[j, :])
                            end
                        end
                    end
                    if opt_D
            ...
            Δ_conj_el .+= conjdelta .* el
            @tensor Δ_conj_Δ[i, j] += conjdelta[i] * delta[j] #this is causing a serious performance penalty. 26 Jun 2025. Uses less RAM but is slower than saving full walk history and computing S at the end.
        end
        r_prev = copy(R)
    end
    el_std_dev = sqrt(sum(abs.(El_walk .- sum(El_walk) / walk_length) .^ 2) / (walk_length - 1))
    return Δ / walk_length,
    Δ_conj_Δ / walk_length,
    Δ_conj_el / walk_length,
    El_walk,
    sum(El_walk) / walk_length,
    v_avg / walk_length,
    t_avg / walk_length,
    el_std_dev,
    acceptance_rate / walk_length,
    R
end
...
function sample_gaussian(
    ...
    U_total = sum(u_pair_mat) / 2
    pix_D, piy_D = calc_pi_D_gaussian(R, orbital_inputs)
    pi_square_D = calc_pi_square_D_gaussian(R, orbital_inputs)
    gradx_u_pair_mat = copy(u_pair_mat)
    grady_u_pair_mat = copy(u_pair_mat)
    laplacian_u_pair_mat = copy(u_pair_mat)
    if comp_el
    ...
    for I in 1:walk_length
        ...
        if accept
            r_diffs = copy(r_diffs_new)
            #update slater stuff
            pix_D_new_coli, piy_D_new_coli = calc_pi_D_gaussian(
                reshape(ri_new, (1, 2)), orbital_inputs
            )
            pi_square_D_new_coli = calc_pi_square_D_gaussian(
                reshape(ri_new, (1, 2)), orbital_inputs
            )
            D_inv = update_D_inv(i, delta_D_new_coli, D_inv) #Sherman-Morrison
            D[:, i] = D_new_coli
...
function sample_opt_gaussian(
    ...
    for I in 1:(walk_length + burn)
        ...
        if accept
            ...
            pix_D[:, i] = pix_D_new_coli
            piy_D[:, i] = piy_D_new_coli
            pi_square_D[:, i] = calc_pi_square_D_gaussian(
                reshape(ri_new, (1, 2)), orbital_inputs
            )
            u_pair_mat = copy(u_pair_mat_new)
            gradx_u_pair_mat[i, :] = gradx_u_pair_row_new_i
            ...
            U_total = copy(U_total_new)
            delta_v = calc_Coulomb_energy_change(
                i, r_prev[i, :], R, η, area, v_long_g, l_vals, g_vals
            )
            T = calc_kinetic_energy(
                pix_D,
                piy_D,
        ...
        if I > burn
            ...
            if (I - burn) == 1
                ...
                if opt_D
                    D_d_l0 = calc_dl0_D_gaussian(R, orbital_inputs)
