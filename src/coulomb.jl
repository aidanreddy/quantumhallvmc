function calcv_short(r::Array{Float64},η::Float64)::Array{Float64}
    return erfc.(r ./(2η)) ./r
end

function calcv_long_g(q::Array{Float64},η::Float64)::Array{Float64}
...
function calc_coulomb_energy_const(ne::Int64,η::Float64,area::Float64,l_vals,g_vals)
    ...
    g_norms=norm.(g_vals)
    l_norms=norm.(l_vals)
    deleteat!(l_norms, findall(x->x==0.,l_norms)) #remove zero because it doesnt enter self image energy
    v_long_g=calcv_long_g(g_norms,η)
    v_const_self= (ne/2)*(sum(calcv_short(l_norms,η))- 1/(η*√π) + sum(v_long_g)/area - 4*√π*η/area)
    v_const_pair= -npair*(2π/area)*(2η/√π) #q=- part of short-range interaction
    return v_const_pair+v_const_self, v_long_g
end

function calc_coulomb_energy(R::Array{Float64},η::Float64,area::Float64,v_long_g::Vector{Float64},v_const::Float64,l_vals,g_vals)
    #everything in units of e²/(ϵ*(length unit))
    #GVals exlcudes  (0,0)
    ne=size(R,1)
    npair=binomial(ne,2)
    rij_vals=[[0.,0.] for i in 1:npair]
    rij_minus_l_vals=[[0.,0.] for i in 1:(npair*length(l_vals))]
    cnt1=1
    cnt2=1
    ...
    v_short=sum(calcv_short(norm.(rij_minus_l_vals),η))
    v_long=0.
...
function calc_coulomb_energy_change(change_ind::Int64,
    ...
    return calc_coulomb_energy_change!(change_ind, ri_old, R, η, area, v_long_g, l_vals, g_vals,
...
function calc_coulomb_energy_change!(change_ind::Int64,
                                  ...
                                  rij_new_minus_l_vals::Vector{Vector{Float64}})::Float64
