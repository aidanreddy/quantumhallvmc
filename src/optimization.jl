...
function opt_wf(
    ...
    area, L1, L2, G1, G2, mesh, g1, g2, RL = geometry_setup(
        A1, A2, N1, N2, rl_cut, B_field, fermi_surface
    )
    γ = norm(L1) / sqrt(2) / π * κ / 3 #this parameter fixed kato cusp condition
    supercell_to_cart = [L1 L2]
    cart_to_supercell = inv(supercell_to_cart) #cartesian to supercell coordinate basis change
    #initialize miscellaneous
    params = Vector{Float64}()
    if opt_J
        params = vcat(params, J_params)
    end
    if opt_D
        params = vcat(params, vec(D_params))
    end
    ...
    #optimize
    R_start_list = [
        Matrix(transpose((supercell_to_cart * transpose(rand(ne, 2) .- 0.5)))) for
        _ in 1:n_walkers
    ]
    ...
    el_list = [0.0 + 0im for _ in 1:n_walkers]
    El_walk_list = [zeros(ComplexF64, walk_length) for _ in 1:n_walkers]
    tl_list = [0.0 + 0im for _ in 1:n_walkers]
    vl_list = [0.0 for _ in 1:n_walkers]
    el_std_dev_list = [0.0 for _ in 1:n_walkers]
    acceptance_rate_list = [0.0 for _ in 1:n_walkers]
    ...
    for i in 1:n_opt #optimization steps
        ...
        dx_u_func(r) = calc_grad_u_sin_spl(r, G1, G2, c, γ)[1]
        dy_u_func(r) = calc_grad_u_sin_spl(r, G1, G2, c, γ)[2]
        laplacian_u_func(r) = calc_laplacian_u_sin_spl(r, G1, G2, c, γ)
        ...
        Threads.@threads for j in 1:n_walkers
            delta_list[j], delta_conj_delta_list[j], delta_conj_el_list[j], El_walk_list[j], el_list[j], vl_list[j], tl_list[j], el_std_dev_list[j], acceptance_rate_list[j], r_stop_list[j] = sample_opt(
                κ,
                ne,
                R_start_list[j],
                burn,
        ...
        for i in 1:n_walkers
            R_start_list[i] = r_stop_list[i]
        end #each walker at each optimization step starts at last position of its previous optimization step walk
        accept_rate = sum(acceptance_rate_list) / n_walkers
        if accept_rate > 0.5 #.574 #settles on .4 after a bit of playing around
            σ *= 1.05
        elseif accept_rate < 0.5 #.574
            σ *= 0.95
        end
        ...
        #update parameters through stochastic reconfiguration method
        tic = time()
        if i < n_opt #don't update params on last cycle so that the last set of params are sampled. 23 Jun 2025t
            ...
            if opt_D
                D_params = reshape(params[(NJ + 1):end], size(D_params))
            end
        end
...
function opt_wf_gaussian(
    ...
    for i in 1:n_opt #optimization steps
        ...
        L0 = D_params[1]
        #build Jastrow functions
        if !B_field #periodic boundary conditions (B=0)
            orbital_func_pbc(R0, r) = gaussian_pbc(r + R0, L0, l_vals, L1, L2)
            orbital_func_pbc_pi(R0, r) = calc_pi_gaussian_pbc(r + R0, L0, l_vals, L1, L2)
            orbital_func_pbc_pi_square(R0, r) = calc_pi_square_gaussian_pbc(
                r + R0, L0, l_vals, L1, L2
            )
            orbital_func_pbc_d_l0(R0, r) = calc_dl0_gaussian_pbc(r + R0, L0, l_vals, L1, L2)
            orbital_inputs = (
                orbital_func_pbc,
                orbital_func_pbc_pi,
                orbital_func_pbc_pi_square,
                orbital_func_pbc_d_l0,
                gaussiansites,
            )
        else #magnetic boundary conditions
            orbital_func_mbc(R0, r) = 
                gaussian_mbc(r + R0, L0, 1.0, l_vals, L1, L2) *
                exp(1im * (r[1] * R0[2] - r[2] * R0[1]) / 2)
            orbital_func_mbc_pi(R0, r) = 
                calc_pi_gaussian_mbc(r + R0, L0, 1.0, l_vals, L1, L2) *
                exp(1im * (r[1] * R0[2] - r[2] * R0[1]) / 2)
            orbital_func_mbc_pi_square(R0, r) = 
                calc_pi_square_gaussian_mbc(r + R0, L0, 1.0, l_vals, L1, L2) *
                exp(1im * (r[1] * R0[2] - r[2] * R0[1]) / 2)
            orbital_func_mbc_d_l0(R0, r) = 
                calc_dl0_gaussian_mbc(r + R0, L0, 1.0, l_vals, L1, L2) *
                exp(1im * (r[1] * R0[2] - r[2] * R0[1]) / 2)
            orbital_inputs = (
                orbital_func_mbc,
                orbital_func_mbc_pi,
                orbital_func_mbc_pi_square,
                orbital_func_mbc_d_l0,
                gaussiansites,
            )
        end
        u_func(r) = calc_u_sin_spl(r, G1, G2, c, γ)
        ...
        tic = time()
        Threads.@threads for j in 1:n_walkers
            delta_list[j], delta_conj_delta_list[j], delta_conj_el_list[j], El_walk_list[j], el_list[j], vl_list[j], tl_list[j], el_std_dev_list[j], acceptance_rate_list[j], r_stop_list[j] = sample_opt_gaussian(
                κ,
                R_start_list[j],
                burn,
                walk_length,
...
function stochasticreconfig!(
    ...
    for i in axes(S, 1)
        S[i, i] += ϵ
    end #for numerical stability of inversion, add some small shift to diagonal
    @assert !any(isnan, S)
    δ .= real.(-τ * inv(real.(S)) * (real.(f) .- (ϵ * μ / τ) .* δ)) #purely real version. I.e. S is fubini-study metric =  Re(QGT) 06 Sep 2025
end
