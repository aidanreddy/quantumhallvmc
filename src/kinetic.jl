function calc_kinetic_energy(
    ...
    T_jastrow = 
        (
            ...
            sum(sum(gradx_u_pair_mat; dims = 2) .^ 2) +
            sum(sum(grady_u_pair_mat; dims = 2) .^ 2) +
            sum(laplacian_u_pair_mat)
        )
    ...
    T_mix = 
        (
            ...
            dot(vec(sum(gradx_u_pair_mat; dims = 2)), pi_D_ratio[:, 1]) +
            dot(vec(sum(grady_u_pair_mat; dims = 2)), pi_D_ratio[:, 2])
        )
